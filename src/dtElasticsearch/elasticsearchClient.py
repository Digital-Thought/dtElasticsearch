...
class ElasticsearchConnection:
    ...
    def install_component_template(self, template_name, template, description=None, version=None, requires_prefix=None):
        if template_name not in self.loaded_component_templates():
            if '_meta' not in template and (description is None or version is None or requires_prefix is None):
                raise Exception(
                    "Template does not contain all required _meta fields [description, version, requires_prefix]")
    ...
    def install_lifecycle_policy(self, policy_name, policy, description=None, version=None, requires_prefix=None):
        if policy_name not in self.loaded_lifecycle_policies():
            if '_meta' not in policy and (description is None or version is None or requires_prefix is None):
                raise Exception(
                    "Policy does not contain all required _meta fields [description, version, requires_prefix]")
    ...
    def install_index_template(self, template_name, template, prefix=None, description=None, version=None,
                               requires_prefix=None):
        if '_meta' not in template and (description is None or version is None or requires_prefix is None):
            raise Exception(
                "Index Template does not contain all required _meta fields [description, version, requires_prefix]")
        elif '_meta' not in template:
            template['_meta'] = {'description': description, 'version': version, 'requires_prefix': requires_prefix}
        ...
        if template['_meta']['requires_prefix']:
            if prefix is None:
                logging.getLogger('elastic').error(
                    'Failed to Index Template {}.  Missing required prefix'.format(template_name))
                raise Exception('Failed to Index Template {}.  Missing required prefix'.format(template_name))
            if not re.compile('^[a-z0-9-]+$').match(prefix) or prefix.endswith('-') or not re.compile('^[a-z]+$').match(
                    prefix[0]):
                logging.getLogger('elastic').error(
                    'Invalid Prefix {}.  Can only contain lowercase letters, numbers and -.  Must not end with - and must start with letter'.format(
                        prefix))
                raise Exception(
                    'Invalid Prefix {}.  Can only contain lowercase letters, numbers and -.  Must not end with - and must start with letter'.format(
                        prefix))
            ...
            template['template']['settings']['index.lifecycle.rollover_alias'] = template['template']['settings'][
                'index.lifecycle.rollover_alias'].replace('<prefix>', prefix)
        ...
        if template_name not in self.loaded_index_templates():
            ...
            if 'composed_of' in template:
                ...
                if len(missing_components) > 0:
                    logging.getLogger('elastic').error(
                        'Missing required template component: {}'.format(str(missing_components)))
    ...
    def is_cluster_healthy(self):
        try:
            ...
            if json['status'] == 'yellow':
                logging.getLogger('elastic').warning(
    ...
    def find_by_id(self, index, _id, alias_index=True):
        if not alias_index:
            ...
            if response['hits']['total']['value'] > 1:
                ...
                return {'_index': response['hits']['hits'][0]['_index'], '_type': '_doc', '_id': _id, 'found': True,
